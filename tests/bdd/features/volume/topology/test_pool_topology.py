...
@scenario('pool-topology.feature', 'Suitable pools which contain volume topology labels')
def test_suitable_pools_which_contain_volume_topology_labels():
    """Suitable pools which contain volume topology labels."""

@scenario('pool-topology.feature', 'Suitable pools which contain volume topology keys only')
...
@given(parsers.parse('a request for a {replica} replica volume with poolAffinityTopologyLabel as {pool_affinity_topology_label} and pool topology inclusion as {volume_pool_topology_inclusion_label}'))
def a_request_for_a_replica_replica_volume_with_poolaffinitytopologylabel_as_pool_affinity_topology_label_and_pool_topology_inclusion_as_volume_pool_topology_inclusion_label(create_request, replica, pool_affinity_topology_label, volume_pool_topology_inclusion_label ):
...
@given(parsers.parse('a request for a {replica} replica volume with poolHasTopologyKey as {has_topology_key} and pool topology inclusion as {volume_pool_topology_inclusion_label}'))
def a_request_for_a_replica_replica_volume_with_poolhastopologykey_as_has_topology_key_and_pool_topology_inclusion_as_volume_pool_topology_inclusion_label(create_request,  replica, has_topology_key, volume_pool_topology_inclusion_label):
...
@when(parsers.parse('the desired number of replica of volume i.e. {replica} here; is {expression} number of the pools containing the label {volume_pool_topology_inclusion_label}'))
def the_desired_number_of_replica_of_volume_ie_replica_here_is_expression_number_of_the_pools_containing_the_label_volume_pool_topology_inclusion_label(create_request, replica, expression, volume_pool_topology_inclusion_label):
...
@then(parsers.parse('the {replica} replica volume creation should {result} and {provisioned} provisioned on pools with labels {pool_label}'))
def the_replica_replica_volume_creation_should_result_and_provisioned_provisioned_on_pools_with_labels_pool_label(create_request, replica, result, provisioned, pool_label):
    ...
    if result == "succeed":
        ...
        pools_names_which_has_given_labels = get_pool_names_with_given_labels(pool_label)
        ...
        assert set(pools_on_which_volume_provisioned).issubset(set(pools_names_which_has_given_labels))
    elif result == "fail":
...
def create_volume_body(replica, volume_pool_topology_inclusion_label):
    ...
    topology = Topology(
                pool_topology=PoolTopology(
                    labelled=LabelledTopology(
                        exclusion={},
                        inclusion={key.strip(): value.strip(), "openebs.io/created-by": "operator-diskpool"},
                    )
                )
            )
