...
@scenario('node-topology.feature', 'Suitable nodes which contain volume topology labels')
def test_suitable_nodes_which_contain_volume_topology_labels():
    """Suitable nodes which contain volume topology labels."""

@scenario('node-topology.feature', 'Suitable nodes which contain volume node topology keys only')
...
def a_control_plane_four_ioengine_instances_twelve_pools():
    ...
    assert node4["spec"]["labels"] == {"zone-us": "us-west-1", "zone-ap": "ap-south-1", "zone-eu": "eu-west-3"}
    


@given(parsers.parse('a request for a {replica} replica volume with nodeAffinityTopologyLabel as {node_affinity_topology_label} and node topology inclusion as {volume_node_topology_inclusion_label}'))
def a_request_for_a_replica_replica_volume_with_nodeaffinitytopologylabel_as_node_affinity_topology_label_and_node_topology_inclusion_as_volume_node_topology_inclusion_label(create_request, replica, node_affinity_topology_label, volume_node_topology_inclusion_label):
...
@given(parsers.parse('a request for a {replica} replica volume with nodeHasTopologyKey as {has_topology_key} and node topology inclusion as {volume_node_topology_inclusion_label}'))
def a_request_for_a_replica_replica_volume_with_nodehastopologykey_as_has_topology_key_and_node_topology_inclusion_as_volume_node_topology_inclusion_label(create_request, replica, has_topology_key, volume_node_topology_inclusion_label):
...
@when(parsers.parse('the desired number of replica of volume i.e. {replica} here; is {expression} number of the nodes containing the label {volume_node_topology_inclusion_label}'))
def the_desired_number_of_replica_of_volume_ie_replica_here_is_expression_number_of_the_nodes_containing_the_label_volume_node_topology_inclusion_label(create_request, replica, expression, volume_node_topology_inclusion_label):
...
@then(parsers.parse('the {replica} replica volume creation should {result} and {provisioned} provisioned on pools with its corresponding node labels {node_label}'))
def the_replica_replica_volume_creation_should_result_and_provisioned_provisioned_on_pools_with_its_corresponding_node_labels_node_label(create_request, replica, result, provisioned, node_label):
    ...
    if result == "succeed":
        ...
        expected_spec = expected_volume_spec(replica, create_request[CREATE_REQUEST_KEY]["topology"] )
        assert str(volume.spec) == str(expected_spec)
        node_name_whose_node_has_given_labels = get_node_names_with_given_labels(node_label)
        ...
        assert set(nodes_on_which_volume_provisioned).issubset(set(node_name_whose_node_has_given_labels))
    elif result == "fail":
    ...
    for key, value in volume_node_topology_inclusion_labels.items():
        if key in node_with_its_node_labels:
            if value == "":
                inc_match = True
                break
            if volume_node_topology_inclusion_labels[key] != node_with_its_node_labels[key]:
                inc_match = False
                break
        else:
            inc_match = False
